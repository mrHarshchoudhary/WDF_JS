...
function Throw1() {
  ...
  if (isNaN(mini) || isNaN(maxi) || mini >= maxi) {
    document.getElementById("result").innerHTML =
      "Invalid range. Ensure Min < Max and both are valid numbers.";
    return;
  }
...
function checkRange() {
  ...
  if (Yguess === num1) {
    document.getElementById(
      "result"
    ).innerHTML = `🎉Correct answer!🎉 The number was ${num1}.`;
    clearFields();
  } else {
    if (diff === 1) {
      document.getElementById(
        "result"
      ).innerHTML = `You're very close! You're just 1 number away!`;
    } else if (diff <= 3) {
